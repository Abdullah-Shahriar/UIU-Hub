  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border bg-card shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-semibold">Schedule Planner</CardTitle>
      ...
      <Card className="border bg-card shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-semibold">Time Preferences</CardTitle>
      ...
      <Card className="border bg-card shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-semibold">Course Selection</CardTitle>
            ...
            <div className="space-y-2">
              {courseSelection.selectedCourses.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {courseSelection.selectedCourses.map((course, index) => (
                    <div key={index} className="bg-primary/10 text-primary px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 border border-primary/20">
                      <span className="font-medium">{course.split(' - ')[0]}</span>
                      <button
                        onClick={() => {
              ...
              <div className="relative">
                ...
                {(isCourseSearchFocused || courseSearchTerm) && getFilteredCourses().length > 0 && (
                  <div className="absolute top-full left-0 right-0 z-50 bg-popover border border-border rounded-lg shadow-lg mt-1 max-h-60 overflow-y-auto">
                    {getFilteredCourses()
                      ...
                      .map((course, index) => (
                        <div
                          key={index}
                          className="flex flex-col px-3 py-2.5 hover:bg-muted cursor-pointer border-b border-border/50 last:border-b-0 transition-colors"
                          onClick={() => {
                            const courseKey = `${course.courseCode} - ${course.title}`;
                            setCourseSelection(prev => ({
                              ...prev,
                        ...
                        >
                          <div className="font-medium text-sm text-foreground">{course.courseCode}</div>
                          <div className="text-xs text-muted-foreground mt-0.5 leading-relaxed">{course.title}</div>
                        </div>
                      ))}
                    {getFilteredCourses().filter(course => !courseSelection.selectedCourses.includes(`${course.courseCode} - ${course.title}`)).length > 10 && (
                      <div className="px-3 py-2 text-xs text-muted-foreground text-center bg-muted/50 border-t border-border/50">
                ...
                {(isCourseSearchFocused || courseSearchTerm) && getFilteredCourses().length === 0 && courseSearchTerm && (
                  <div className="absolute top-full left-0 right-0 z-50 bg-popover border border-border rounded-lg shadow-lg mt-1 p-3">
              ...
              {courseSelection.prioritizedFaculties.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {courseSelection.prioritizedFaculties.map((faculty, index) => (
                    <div key={index} className="bg-muted text-foreground px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 border border-border">
                      <span className="font-medium">{faculty.split('(')[0].trim()}</span>
                      <button
                        onClick={() => {
                          setCourseSelection(prev => ({
                            ...prev,
                  ...
                  <div className="absolute top-full left-0 right-0 z-50 bg-popover border border-border rounded-lg shadow-lg mt-1 max-h-60 overflow-y-auto">
                    {getFilteredFaculties()
                      ...
                      .map((faculty, index) => (
                        <div
                          key={index}
                          className="px-3 py-2.5 hover:bg-muted cursor-pointer border-b border-border/50 last:border-b-0 transition-colors"
                          onClick={() => {
                            setCourseSelection(prev => ({
                              ...prev,
                        ...
                        >
                          <div className="font-medium text-sm text-foreground">{faculty}</div>
                        </div>
                      ))}
                  </div>
                )}
      ...
      {generatedSchedules.length > 0 && (
        <div className="space-y-6">
          ...
          {generatedSchedules.map((schedule, index) => (
            <Card key={schedule.id} className="border bg-card shadow-sm">
              <CardHeader className="px-4 sm:px-6">
                {/* Mobile Layout */}
                <div className="flex flex-col gap-2 sm:hidden">
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-base font-semibold">Schedule {index + 1}</span>
                    <Button
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="bg-muted text-foreground px-2 py-0.5 rounded border border-border">
                ...
                <div className="hidden sm:flex sm:justify-between sm:items-center gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-semibold">Schedule {index + 1}</span>
                    <span className="bg-muted text-foreground px-2 py-0.5 rounded text-sm border border-border">
                      {schedule.totalDays} days/week
                    </span>
                    {schedule.missingFaculties.length === 0 && (
                ...
                {schedule.missingFaculties.length > 0 && (
                  <CardDescription className="text-destructive text-xs sm:text-sm mt-2">
                    Missing preferred faculties: {schedule.missingFaculties.join(", ")}
                  </CardDescription>
                )}
              </CardHeader>
              <CardContent className="px-4 sm:px-6">
                  ...
                  <table className="w-full border-collapse min-w-[640px]">
                    <thead>
                      <tr className="bg-muted/50">
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Days</th>
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Course</th>
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Section</th>
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Faculty</th>
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Time</th>
                        <th className="border-b px-3 py-2.5 text-left font-semibold text-sm">Room</th>
                      </tr>
                    </thead>
                    <tbody>
                      {schedule.courses
                        ...
                        .map((course, courseIndex) => (
                          <tr key={`course-${courseIndex}`} className="border-b hover:bg-muted/30 transition-colors">
                            ...
                            <td className="px-3 py-2.5 text-sm">
                              ...
                              <div className="text-xs text-muted-foreground line-clamp-2">{course.title}</div>
                            </td>
                            <td className="px-3 py-2.5 text-sm">{course.section}</td>
                            <td className="px-3 py-2.5 text-sm">
                              <div className={courseSelection.prioritizedFaculties.includes(`${course.facultyName} (${course.facultyInitial})`) ? 'text-primary font-medium' : ''}>
                                {course.facultyName === "TBA" ? "TBA" : `${course.facultyName} (${course.facultyInitial})`}
                              </div>
                            </td>
                            <td className="px-3 py-2.5 text-sm whitespace-nowrap">{course.time1}</td>
                            <td className="px-3 py-2.5 text-sm">{course.room1}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm">
                  <div className="bg-muted/50 p-2 sm:p-3 rounded-md border border-border/50">
                    <span className="font-medium text-muted-foreground">Courses </span>
                    <span className="font-bold text-foreground">{schedule.courses.length}</span>
                  </div>
                  <div className="bg-muted/50 p-2 sm:p-3 rounded-md border border-border/50">
                  ...
                  <div className="bg-muted/50 p-2 sm:p-3 rounded-md border border-border/50">
                    ...
                    <span className="font-bold text-foreground">{schedule.totalDays}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
