    ...
    <Card className="w-full border bg-card shadow-sm">
      <CardHeader className="px-4 sm:px-6">
        <CardTitle className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <span className="text-base sm:text-lg font-semibold">Course Selection</span>
          <div className="flex gap-2 w-full sm:w-auto">
            <Button 
              variant="ghost" 
              size="sm" 
              onClick={handleClear} 
              className="flex-1 sm:flex-none text-xs sm:text-sm text-muted-foreground hover:text-foreground"
            >
              Clear
            </Button>
            <Button variant="ghost" size="sm" onClick={() => onDelete(cardData.id)} className="flex-1 sm:flex-none text-xs sm:text-sm text-destructive hover:text-destructive hover:bg-destructive/10">
              Delete
            </Button>
          </div>
        </CardTitle>
      ...
      <CardContent className="space-y-4 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="space-y-2 sm:col-span-2 lg:col-span-1">
            ...
            <div className="relative">
              <Input
                ...
                className={`w-full text-xs sm:text-sm ${cardData.selectedCourse ? 'bg-primary/5 border-primary/30' : ''}`}
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                } ${cardData.selectedFaculty ? 'bg-primary/5 border-primary/30' : ''}`}
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger className={`text-xs sm:text-sm ${!cardData.selectedFaculty ? "opacity-50" : ""} ${cardData.selectedDay ? 'bg-primary/5 border-primary/30' : ''}`}>
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger className={`text-xs sm:text-sm ${!cardData.selectedDay ? "opacity-50" : ""} ${cardData.selectedTime ? 'bg-primary/5 border-primary/30' : ''}`}>
        ...
        {cardData.finalSection && (
          <div className={`space-y-3 p-3 sm:p-4 rounded-lg border ${
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                {sectionPlans.length === 1 ? (
                  ...
                  >
                    <SelectTrigger className="h-8 text-xs w-full sm:w-[100px]">
                      ...
                      {sectionPlans.map(plan => {
                        ...
                        return (
                          <SelectItem 
                            key={plan.id} 
                            value={plan.id} 
                            className={wouldConflict ? 'text-destructive' : ''}
            ...
            {/* Duplicate Warning */}
            {isDuplicate && (
              <div className="bg-amber-500/5 border border-amber-500/20 p-2 sm:p-3 rounded-md">
                <div className="font-medium text-xs sm:text-sm text-amber-700 dark:text-amber-400">
                  This exact course section is already selected in another card.
                </div>
              </div>
            )}
            ...
            {/* Conflict Details */}
            {uniqueConflicts.length > 0 && (
              <div className="bg-destructive/5 border border-destructive/20 p-2 sm:p-3 rounded-md">
                <div className="font-medium text-xs sm:text-sm text-destructive mb-2">
                ...
                {uniqueConflicts.map((conflictInfo, index) => (
                  <div key={index} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mb-2 last:mb-0">
                    <div className="flex-1 text-xs sm:text-sm text-destructive/80">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="text-xs px-2 py-1 h-auto whitespace-nowrap w-full sm:w-auto"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
              <div className="space-y-1 text-muted-foreground">
                <div><span className="font-medium text-foreground">Section:</span> {cardData.finalSection.section}</div>
                <div><span className="font-medium text-foreground">Credit:</span> {cardData.finalSection.credit}</div>
              </div>
              <div className="space-y-1 text-muted-foreground">
                <div><span className="font-medium text-foreground">Time:</span> {cardData.finalSection.time1}</div>
                <div><span className="font-medium text-foreground">Room:</span> {cardData.finalSection.room1}</div>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border bg-card shadow-sm">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-md border border-border/50">
            <div className="font-medium mb-1 text-foreground">Search Tips</div>
            <div>Use course codes (CSE 2218), abbreviations (DSA, DM), or numbers for Roman numerals (1 for I)</div>
          </div>
        </CardContent>
      </Card>
      ...
      <Button 
        onClick={addCard} 
        variant="outline" 
        className="w-full border-dashed border-2 hover:border-primary/50 hover:bg-primary/5 h-10 sm:h-12 text-xs sm:text-sm text-muted-foreground hover:text-foreground transition-colors"
      >
